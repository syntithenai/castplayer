...
export default  class CastPlayerComponent extends Component {
    ...
    constructor(props) {
		...
		this.seekOnRemoteLoad = this.seekOnRemoteLoad.bind(this)
		...
		this.pauseCallback = this.pauseCallback.bind(this)
	...
	shouldComponentUpdate(props) {
		...
		// if cast status changes 
		if (props.casting != this.props.casting) {
			console.log(['CASTING CHANGE',props.casting])
			// cast player
			if (props.casting) {
				// hide local player
				if (this.mediaRef.current) {
					this.mediaRef.current.style.visibility = 'hidden'
				}
			// local player
			} else {
				// reload local player
				if (this.mediaRef.current) {
					this.mediaRef.current.style.visibility = 'visible'
					this.mediaRef.current.src = props.media.url;
					this.mediaRef.current.load()
				}
			}
		}
	...
	seekOnRemoteLoad(msg) {
		//console.log(['SEEK ON LOAD',msg])
		let that = this;
		// assume that canSeek comes after canPlay event
		if (msg.field === 'canSeek' && msg.value === true) {
			console.log(['REMOTEPLAYERCHANGE',JSON.stringify(msg)])
			// clear the on off listener
			this.remotePlayerController.removeEventListener(
				cast.framework.RemotePlayerEventType.ANY_CHANGE,
				that.seekOnRemoteLoad
			)
			//this.remotePlayerController.currentMediaProg
			//this.remotePlayerController.seekTo()
			//this.playerHandler.seekTo(this.props.seekTo);
			var player = new cast.framework.RemotePlayer();
			var playerController = new cast.framework.RemotePlayerController(player);
			console.log(['REMOATE SEEK',this.props.seekTo]);
			player.currentTime = this.props.seekTo;
	...
	setupRemotePlayer = function () {
		...
		this.remotePlayerController.addEventListener(cast.framework.RemotePlayerEventType.IS_PAUSED_CHANGED,this.pauseCallback)
		this.remotePlayerController.addEventListener(
			cast.framework.RemotePlayerEventType.IS_PAUSED_CHANGED,
			this.pauseCallback
		);

		this.remotePlayerController.addEventListener(cast.framework.RemotePlayerEventType.IS_MUTED_CHANGED,this.muteCallback)
		this.remotePlayerController.addEventListener(
		...
		this.remotePlayerController.addEventListener(cast.framework.RemotePlayerEventType.VOLUME_LEVEL_CHANGED,this.volumeCallback)
		this.remotePlayerController.addEventListener(
		...
		// create an object satisfying PlayerTarget interface
		// play/pause/stop/disconnect/load/getCurrentMediaTime/getCurrentMediaDuration,seekTo,mute,unMute,isMuted
		...
		// save local volume and set UI volume from remote player
		this.localVolume = this.props.volume
		this.props.onVolume(this.remotePlayer.volumeLevel);
	...
	setupLocalPlayer = function (alreadyLoaded) {
		...
		if (localPlayer) {
			...
			if (this.props.onError) {
				localPlayer.removeEventListener('error', this.props.onError);
			...
			this.playerHandler.setTarget(playerTarget);
			if (!alreadyLoaded) { 
				localPlayer.removeEventListener('loadeddata', function(e) {that.onMediaLoadedLocally.bind(that)()});
				localPlayer.addEventListener('loadeddata', function(e) {that.onMediaLoadedLocally.bind(that)()});
			} else {
				this.playerHandler.loaded(this.startPlayingOnLoad);
			}
			...
			if (this.localVolume != null) {
				console.log(['RESTORE LOCAL VOLUME',this.localVolume])
				this.props.onVolume(this.localVolume)
			}
